...
static PFN_Juice_ResolveHandle g_pfnResolve;
...
static BOOL CALLBACK init_juice_shared_memory(INIT_ONCE *once, void *param, void **ctx)
{
    ...
    g_pfnResolve = (PFN_Juice_ResolveHandle)GetProcAddress(hIcd, "Juice_ResolveHandle");
...
{
    uint64_t key = (uint64_t)(uintptr_t)handle;

    ensure_initialized();

    if (g_pfnResolve)
    {
        struct juice_resolve_result result;
        memset(&result, 0, sizeof(result));
        if (g_pfnResolve(handle, &result) && result.hasTextureMetadata &&
            result.textureMetadataSize > 0 && result.textureMetadataSize <= buf_size)
        {
            memcpy(buf, result.textureMetadata, result.textureMetadataSize);
            if (metadata_size)
                *metadata_size = result.textureMetadataSize;
            INFO("vkd3d_get_shared_metadata: OK via ResolveHandle handle=%#"PRIx64".\n", key);
            return true;
        }
        WARN("vkd3d_get_shared_metadata: ResolveHandle returned no metadata for handle=%#"PRIx64".\n", key);
        return false;
    }
